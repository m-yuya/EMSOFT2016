...
Autonomous driving has attracted attention since the November 3, 2007 DARPA Urban Challenge \url{urmson2008autonomous}.
The Robot Operating System (ROS) \url{quigley2009ros}, \url{rosorg} is open-source middleware that has undergone rapid development and has been widely used for robotics applications (e.g., autonomous driving systems).
The ROS enhances productivity \url{cousins2010sharing}, providing publish/subscribe transport, multiple libraries (e.g., OpenCV \url{opencv} and the Point Cloud Library (PCL) \url{pcl}), and tools to help software developers create robotics applications.
...
This critical problem has been considered by many research communities and various solutions have been proposed and evaluated \url{rosc@roscon2013}, \url{uros@roscon2013}, \url{wei2015rt}.
However, these solutions are insufficient to address the ROS's limitations for real-time embedded systems.

To satisfy the needs of the now-broader ROS community, the ROS will undergo a significant upgrade to ROS2 \url{ros2_iccps2016}. 
...
To satisfy the requirements of these new use cases, the existing version of ROS (hereinafter ROS1) will be reconstructed to improve user-interface APIs and incorporate new technologies, such as Data Distribution Service (DDS) \url{pardo2003omg} \url{schlesselman2004omg}, Zeroconf, Protocol Buffers, ZeroMQ, Redis, and WebSockets. 
...
Several DDS implementations have been evaluated and validated by researchers \url{xiong2010evaluating} \url{sierla2003evaluation} and DDS vendors.
...
To the best of our knowledge, this is the first study to explore ROS2 performance.
 ...
 Section \ref{sec:orgheadline1} provides background information and describe the ROS and DDS system models. 
...
\begin{figure}[t]
\centering
\includegraphics[width=.9\linewidth]{../figure/ros_architecture.eps}
...
In the left side of Figure \ref{fig:orgparagraph1}, ROS1 provides complete implementations, including the communication system, TCPROS/UDPROS. 
...
\begin{figure}[t]
\centering
\includegraphics[width=.8\linewidth]{../figure/ros_pubsub_fig.eps}
...
\vspace{-4mm}
The DDS specification \url{dds1.4} is defined for a publish/subscribe data-distribution system by the Object Management Group (OMG). 
The OMG manages the definitions and standardized APIs; however the OMG hide the details of implementation. Several implementations have been developed by different vendors (e.g., RTI \url{rti_connext} and PRISMTECH \url{ospl_dds_community}).
...
In DDS, each process that constructs applications is called a \emph{participant}, which corresponds to a \emph{node} in the ROS. 
...
\begin{figure}[t]
\centering
\includegraphics[width=.8\linewidth]{../figure/DDS_model.eps}
...
In DDS, there are many \emph{QoS Policies}, which ROS2 should support to extend its capabilities.
\vspace{1mm}  

\begin{figure}[t]
\centering
\includegraphics[width=.8\linewidth]{../figure/DDS_qos.eps}
...
\hline
DEADLINE & \small A \emph{DataWriter} and a \emph{DataReader} must update data at least once every deadline period.\\
\hline
HISTORY & \small This controls whether the data transport should deliver only the most recent value, attempt to deliver all intermediate values, or attempt to deliver something in between (configurable via the \texttt{depth} option).\\
\hline
RELIABILITY & \small In \texttt{BEST\_EFFORT}, data transport is executed as soon as possible. However, some data may be lost if the network is not robust. In \texttt{RELIABLE}, missed samples are retransmitted. Therefore, data delivery is guaranteed.\\
\hline
DURABILITY & \small With this policy, the service attempts to keep several samples so that they can be delivered to any potential late-joining \emph{DataReader}. The number of saved samples depends on HISTORY. This option has several values, such as \texttt{VOLATILE} and \texttt{TRANSIENT\_LOCAL}.\\
\hline
\end{tabular}
\end{table}
...
After this transaction, a \emph{DataWriter} connects to a \emph{DataReader} using the Real-Time Publish/Subscribe (RTPS) protocol \url{rtps2.2} in distributed systems. 
...
The scope of the transferred data size is 256 B to 4 MB because large image data (e.g., 2 MB) and point cloud data (.pcd) are frequently used in ROS applications, such as an autonomous driving system \url{kato2015open}.
...
We compare three DDS implementations, i.e., Connext \url{rti_connext}, OpenSplice \url{ospl_dds_community}, and FastRTPS \url{fastrtps}. 
...
For precise evaluation and real-time requirements, \emph{nodes} follow \emph{SCHED\_FIFO} \url{garg2009real} and the \emph{mlockall} system call. 
...
\begin{figure}[t]
\centering
\includegraphics[width=.9\linewidth]{../figure/eval_situation.eps}
...
\begin{figure}[t]
\centering
\includegraphics[width=.9\linewidth]{../figure/eval_method.eps}
\caption{\label{fig:orgparagraph6}
Evaluation method for (3-a).}
\end{figure}
...
Communication between ROS1 and ROS2 requires a \texttt{ros\_bridge}, a bridge-node that converts \emph{topics} for DDS. 
...
The \texttt{ros\_bridge} program has been released by the OSRF.
Note that a \texttt{best-effort policy} is only used when using a \texttt{ros\_bridge} because a \texttt{ros\_bridge} does not support the \texttt{RELIABLE} policy in the \emph{QoS Policy}. 
...
The difference in end-to-end latencies between ROS1 and ROS2 is much less than the difference between \texttt{remote} and \texttt{local} cases.
...
In contrast, the latencies in the \texttt{remote} cases grow sharply from 4 KB, as shown in Figures \ref{fig:remote_local_small_plot} and \ref{fig:remote_local_plot}.
...
\vspace{-4mm}
In addition to the ROS, the Robot Technology Middleware (RTM) \url{ando2005rt} is well known and widely used for robotics development. 
...
\url{chishiro2009extended} extends RTC for real-time requirements using GIOP rather than CORBA.
...
Using lightweight CORBA and libraries, \url{ikezoeopenrt} enables RTM to run on VxWorks, which is an RTOS.
Nonetheless,  \url{ikezoeopenrt} din not consider real-time requirements. 
Furthermore, it uses global variables and cannot run on distributed systems.

\textbf{RTM-TECS:} RTM-TOPPERS Embedded Component Systems (TECS) \url{rtmtecs2015} proposed a collaboration framework of two component technologies, i.e., RTM and TECS. 
TECS \url{azumi2015mruby} has been added to RTM to satisfy real-time processing requirements. 
\url{rtmtecs2015} adapted RPC and one-way data transport between TECS components and RTC. 
Thus, RTM-TECS enhances the capability for real-time embedded systems.

\textbf{rosc:} 
rosc \url{rosc@roscon2013} is a portable and dependency-free ROS client library in pure C that supports small embedded systems and any OS. rosc was motivated by a bare-metal, low-memory reference scenario, which ROS2 also targets. While rosc is available as an alpha release, it is in development and has not been updated since 2014.

\textbf{\(\mu\)ROS:} 
\(\mu\)ROS \url{uros@roscon2013} \url{uros} is a lightweight ROS client that can run on 32-bit micro-controllers, such as the ARM Cortex-M. 
...
ROS-Industrial \url{rosindustrial} is an open-source project that extends the advanced capabilities of ROS software to manufacturing. 
...
\textbf{nodelet:} 
\emph{nodelet} is efficient \emph{node} composition for ROS1 to improve of performance. 
...
\textbf{RT-ROS:} 
RT-ROS \url{wei2015rt} provides an integrated real-time/non-real-time task execution environment. 
...
In future, we will evaluate real-time applications such as an autonomous driving vehicle \url{kato2015open} \url{saito2014fusion}, as case studies using ROS2.
