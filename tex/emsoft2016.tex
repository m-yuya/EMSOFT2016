...
\begin{abstract}
  ...
  However, the ROS is not suitable for real-time embedded systems because it does not satisfy real-time requirements and only runs on a few OSs.
...
The ROS is built almost entirely from scratch and has been maintained by Willow Garage \cite{willowgarage} and Open Source Robotics Foundation (OSRF) \cite{osrf} since 2007. 
...
Performance means latency characteristics, throughput and distributed capability.
...
This data does not route through the master. 
...
Developers can select an appropriate DDS implementation from a variety of DDS vendors.
...
It is hard to maintain divided packets with \emph{QoS Policy} by default API.
...
This is because ROS1 and ROS2 divide a \emph{message} into 15 KB packets to transmit data through Ethernet.
...
\label{sec:local}
The latenciy characteristics differ in the cases of small and large data.
...
For example, in 1 MB, subscriber 5 is about twice as much as subscriber 1.
...
However, ROS2 latencies significantly depend on the number of packets.
...
Although we cannot say that latency variance of ROS1 is larger than one of ROS2 due to the difference of the scale, Figures \ref{fig:ros1_multi}, \ref{fig:ospl_multi}, and \ref{fig:ospl_multi_128K} prove ROS2 \emph{message} publication is more fair to multiple \emph{subscriber-nodes} than ROS1 one.
...
From Section \ref{sec:latency}, the latencies are caused by two data conversions for DDS and DDS transaction.
DDS end-to-end latencies are constant until \emph{message} data size is lower than maximum packet size (64 KB) as shown in Figure \ref{fig:local_small_plot}.
On the other hand, as one large \emph{message} is divided into several packets, the latencies sharply increase as show in Figures \ref{fig:local_plot} and \ref{fig:ospl_multi}.
...
In the \texttt{remote} case, although overhead latencies are trivial, we must consider throughput for bandwidth.
...
\setlength{\baselineskip}{10.2pt}      % 行間の直接指定
